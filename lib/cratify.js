...
$.fn.cratify = function(styleOptions, connectionOptions, session){
    ...
    return this.each(function(){
        ...
        this.m = new Model(connectionOptions, metadata);
        
        var vss = new VSignalingState(this.m.signaling, signalingState);
        var vns = new VNetworkState(networkState);
        var vl = new VLink(linkContainer, this, id);
        var vm = new VMetadata(this.m, buttonFile);

        var csb = new CShareButton(this.m, shareButton, vss, vl);
        var clb = new CLaunchButton(this.m, shareButtons[0], vl);
        var cab = new CAnswerButton(this.m, shareButtons[1], vl);
        var chb = new CHandshakeButton(this.m, shareButtons[2], vl);
        var cns = new CNetworkState(this.m, vns);
        
        var ce = new CEditor(this.m, divEditor, editor);
