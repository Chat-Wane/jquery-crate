...
$.fn.cratify = function(styleOptions, connectionOptions, session){
    ...
    return this.each(function(){
        ...
        var headerRightRightRight = jQuery('<div>').appendTo(headerContainer)
            .addClass('pull-right')
            .css('padding-top', '10px')
            .css('padding-bottom', '10px')
            .css('height', '34px');
        ...
        this.header = headerRightRight;
        
        // Contains data about the document
        this.closeButton =  jQuery('<button>').appendTo(headerRightRightRight)
            .attr('type', 'button')
            .addClass('close')
            .css('color', 'white')
            .append(jQuery('<span>')
        ...
        this.model = new Model(connectionOptions, metadata);
        
        var vss = new VSignalingState(this.model.signaling, signalingState);
        var vns = new VNetworkState(networkState);
        var vl = new VLink(linkContainer, this, id);
        var vm = new VMetadata(this.model, buttonFile);

        var csb = new CShareButton(this.model, shareButton, vss, vl);
        var clb = new CLaunchButton(this.model, shareButtons[0], vl);
        var cab = new CAnswerButton(this.model, shareButtons[1], vl);
        var chb = new CHandshakeButton(this.model, shareButtons[2], vl);
        var cns = new CNetworkState(this.model, vns);
        var ccb = new CCloseButton(this.model, this.closeButton, this);
        
        var ce = new CEditor(this.model, divEditor, editor);
