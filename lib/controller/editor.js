...
function EditorController(model, viewEditor){
    ...
    if (!insertRemoveOp){
		...
		model.core.caretMoved({
			start: editor.getSession().getDocument().positionToIndex(sel.selectionLead),
			end: editor.getSession().getDocument().positionToIndex(sel.selectionAnchor)
		});
    }
        ...
        marker.update = function(html, markerLayer, session, config) {
            ...
            for (var i = 0; i < cursors.length; i++) {
				...
				if (range.start.row == range.end.row){
                    ...
                    var left = markerLayer.$padding + startScreenPos.column * width;
					...
					width = config.characterWidth * range.end.column;
					...
					if (range.end.row - range.start.row > 1){
						height = config.lineHeight * (range.end.row - range.start.row - 1);
						top = markerLayer.$getTop(startScreenPos.row + config.lineHeight, config);
						left = markerLayer.$padding;
						code += "<div class='remoteCaret' style='" +
							"background-color:" + this.color + ";" +
							"height:" + height + "px;" +
