...
function EditorController(model, viewEditor){
    ...
    model.core.on('remoteCaretMoved', function(range, origin){
        ...
        console.log('caretMoved from ' + origin +
                    ' for indexes positions ' + range.start + ";" + range.end);
        if (editor.session.remoteCarets[origin] !== undefined){
            ...
            var marker = new Marker(editor.session, range);
