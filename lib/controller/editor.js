...
function EditorController(model, viewEditor){
    ...
    editor.getSession().on('change', function(e) {
		...
		remoteCaretsUpdate_removeCase(begin, e.data.text.length);
                break;
            case 'insertLines':
			...
			marker.update = function(html, markerLayer, session, config) {
				...
				for (var i = 0; i < cursors.length; i++) {
						...
						if (startScreenPos.row == endScreenPos.row){//!range.isMultiLine()){
							...
							var width = config.characterWidth * (endScreenPos.column - startScreenPos.column) +1; // TODO fix this
    ...
    function remoteCaretsUpdate_removeCase(index, length){
	var change = false;
    ...
    function remoteCaretsUpdate_insertCase(index){
    };
