...
function ShareButtonController(model, shareBtn,
    ...
    this.model = model;
    this.signalingView = signalingView;
...
ShareButtonController.prototype.startJoining = function(session){
    var socket = this.model.signaling.startJoining(session);
    this.signalingView.setState('waitSignaling');

    var self = this;
    socket.on('connect',
              function(){ self.signalingView.setState('waitSharer'); });
};
